...
def find_peaks(heatmap, threshold):
    ...
    :param heatmap:
    :param threshold:
    :return: heatmap上高于threshold的不连续区域(island)的所有峰(peak)
    """
    truth_islands = heatmap > threshold
    segmented_islands, island_hierarchy = mark_islands(truth_islands)  # 分割并标记不连续区域，然后返回岛层次结构字典。
...
class Skeletonizer:
    def __init__(self, kpts, pafs):
        self.kpts = kpts.numpy()
        self.pafs = pafs.numpy()
    ...
    def _localize_potential_kpts(self):
        potential_kpts = {}
        for kpt_name, kpt in cfg.KEYPOINTS_DEF.items():
            kpt_idx = kpt["idx"]
            kpt_heatmap = self.kpts[..., kpt_idx]
            peaks = find_peaks(kpt_heatmap, cfg.KEYPOINTS_HEATMAP_THRESHOLD)
    ...
    def _create_joints(self, potential_kpts: dict):
        ...
        joints = {}
        x_paf_offset = len(cfg.JOINTS_DEF)
        for joint_name, joint in cfg.JOINTS_DEF.items():  # work by joints definitions
    ...
    def _joint_matching(self, joint_candidates: list, num_joints: int):
        ...
        filtered_candidates = filter(lambda jc: jc[0] > cfg.JOINT_ALIGNMENT_THRESHOLD, joint_candidates)
    ...
    def _build_skeletons(self, joint_lists):
        ...
        for joint_name, joints_coords in joint_lists.items():
            start_kpt_name = cfg.JOINTS_DEF[joint_name]["kpts"][0]
            end_kpt_name = cfg.JOINTS_DEF[joint_name]["kpts"][1]
